...
**改动内容**：
- `release-runtime.yml`：改为安装 `.[web,anthropic,mcp]`，PyInstaller 增加 `--collect-submodules mcp` 与 `--copy-metadata mcp`，并扩展 “Verify frozen provider SDKs” 步骤——若缺少 `mcp-*.dist-info` 则直接让构建失败（防止再次悄悄回归）。
