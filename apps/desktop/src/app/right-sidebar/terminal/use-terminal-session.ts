...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  const sessionDisposersRef = useRef<Array<() => void>>([])
  const startSessionRef = useRef<((nextCwd: string) => void) | null>(null)
  ...
  useEffect(() => {
    ...
    const disposeSession = () => {
      const disposers = sessionDisposersRef.current.splice(0)

      disposers.forEach(run => run())

      const id = sessionIdRef.current
      sessionIdRef.current = null
      activeSessionCwdRef.current = null
    ...
    const startSession = (nextCwd: string) => {
      if (disposed) {
        return
      }
      ...
      void terminalApi
        .start({ cols: term.cols, cwd: nextCwd, rows: term.rows })
        .then(session => {
          ...
          activeSessionCwdRef.current = nextCwd
          lastSentSize = { cols: term.cols, rows: term.rows }
          ...
          sessionDisposersRef.current.push(
  ...
  useEffect(() => {
    ...
    if (!start || activeSessionCwdRef.current === cwd) {
      return
    }
